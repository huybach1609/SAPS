...
const ParkingHistoryDetail: React.FC = () => {
  const { parkingLotId, sessionId } = useParams<{
    parkingLotId: string;
    sessionId: string;
  }>();
  ...
  useEffect(() => {
    ...
    setError(null);
    axios
      .get(`${apiUrl}/api/ParkingSession/${parkingLotId}/${sessionId}`)
      .then((res) => {
        setSession(res.data);
        setLoading(false);
      })
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-800">
                  {session.vehicle?.licensePlate} ({session.vehicle?.brand}{" "}
            ...
            <div>
              ...
              <div className="bg-gray-50 rounded-lg p-3 text-gray-800">
                {session.exitDateTime
                  ? formatDateTime(session.exitDateTime)
                  : "N/A"}
              </div>
            </div>
        ...
        <div className="p-6">
                  ...
                  >
                    ...
                    <CardFooter className="justify-between before:bg-white/40 border-white/20 border-1 overflow-hidden py-1 absolute before:rounded-xl rounded-large bottom-1 w-[calc(100%_-_8px)] shadow-small ml-1 z-10">
                      <div>
                        ...
                        <div className="text-xs text-background">
                          {session.exitDateTime
                            ? `${formatDateTime(session.exitDateTime)} ${formatTime(session.exitDateTime)}`
                            : "N/A"}
                        </div>
                      </div>
                      <a
          ...
          <div className="mt-4 p-4 bg-blue-50 rounded-lg">
            <div className="flex items-center gap-2 text-blue-800">
              ...
              <p className="text-sm">
                <strong>Image Information:</strong> All images are automatically
                captured by the AI recognition system. Images are stored
                securely and can be used for verification purposes. Click on any
            ...
            <ModalFooter>
              <div className="mt-2 text-gray-700 text-center">
                ...
                <div className="text-xs">
                  {time ? `${formatDateTime(time)} ${formatTime(time)}` : ""}
                </div>
              </div>
            </ModalFooter>
          </>
