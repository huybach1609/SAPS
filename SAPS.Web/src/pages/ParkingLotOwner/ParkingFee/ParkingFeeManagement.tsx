...
import { Plus, Trash2, Clock, Car, Edit2, BikeIcon } from "lucide-react";
import {
  Table,
  TableHeader,
  TableColumn,
...
const ParkingFeeManagement: React.FC = () => {
  ...
  const [editingSchedule, setEditingSchedule] =
    useState<ParkingFeeSchedule | null>(null);
  ...
  const handleDeleteSchedule = async (id: string) => {
    try {
      ...
      if (
        !confirm("Are you sure you want to delete this parking fee schedule?")
      ) {
        return;
      }
                ...
                {errorModal.messages && errorModal.messages.length > 0 && (
                  <ul className="mt-2 list-disc list-inside text-sm text-danger">
                    {errorModal.messages.map((msg, idx) => {
                      ...
                      return (
                        <li key={idx}>
                          {formatDays(schedule.dayOfWeeks ?? [])} |{" "}
                          {minutesToTime(schedule.startTime)} -{" "}
                          {minutesToTime(schedule.endTime)}
...
const FeeSchedulesTab: React.FC<{
  ...
  const [selectSchedule, setSelectSchedule] =
    useState<ParkingFeeSchedule | null>(null);
              ...
              <TableRow key={schedule.id}>
                <TableCell>
                  <div className="flex items-center gap-1 text-sm ">
                    ...
                    {minutesToTime(schedule.startTime || 0)} -{" "}
                    {minutesToTime(schedule.endTime || 1440)}
                  </div>
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-1 text-sm ">
                ...
                <TableCell>
                  <div className="text-sm ">
                    ...
                    <div className="text-xs ">
                      +{(schedule.additionalFee || 0).toFixed(0)}đ per{" "}
                      {schedule.additionalMinutes || 60}min
                    </div>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="text-sm ">
                    {Array.isArray(schedule.dayOfWeeks) &&
                    schedule.dayOfWeeks.length > 0
                      ? schedule.dayOfWeeks
                          .map(
                            (idx) =>
                              [
